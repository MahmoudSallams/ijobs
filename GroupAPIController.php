...
class GroupAPIController extends AppBaseController
{
    ...
    public function inviteMember($group_id,$profile_id)
    {
        /** @var Group $group */
        ...
        if (empty($group)) {
            ...
            DB::table('group_users')->insert(['group_id' => $group_id, 'profile_id' => $profile_id, 'status' => 0]);
            return $this->sendResponse(array('Done'=>'True'), 'Invitation Sent');
        }
    }

    public function blockMember($group_id,$profile_id)
    {
        /** @var Group $group */
        $group = $this->groupRepository->findWithoutFail($group_id);
        if (empty($group)) {
            ...
            DB::table('group_users')->where('group_id',$group_id)->where('profile_id',$profile_id)->update(array('status'=>-1));
            return $this->sendResponse(array('Done'=>'True'), 'Invitation Blocked');
        }
    }

    public function reinviteMember($group_id,$profile_id)
    {
        /** @var Group $group */
        $group = $this->groupRepository->findWithoutFail($group_id);
        if (empty($group)) {
