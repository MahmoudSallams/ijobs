...
class ContactAPIController extends AppBaseController
{
    ...
    public function updateByjob($id, UpdateContactAPIRequest $request)
    {
        $input = $request->all();

        /** @var Contact $contact */
        //$contact = $this->contactRepository->findWithoutFail($id);
        $contact=Contact::where('job_id', $id)->firstOrFail();
        if (empty($contact)) {
            return $this->sendError('Contact not found');
        }

        $contact = $this->contactRepository->update($input, $contact->id);
